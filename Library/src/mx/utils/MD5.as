package mx.utils{
	...
	public class MD5 {
		...
		private static function hex_md5(src:String):String {
			...
			byteArray.writeUTFBytes(src);
			return binl2hex(core_md5(str2binl(byteArray), byteArray.length*8));
		}
		...
		private static function str2binl(byteArray:ByteArray):Array {
			var bin:Array = new Array();
			var mask:Number = (1 << 8)-1;
			for (var i:Number = 0; i<byteArray.length*8; i += 8) {
				bin[i >> 5] |= (byteArray[i/8] & mask) << (i%32);
			}
			return bin;
		}
		private static function binl2hex(binarray:Array):String {
			...
			for (var i:Number = 0; i<binarray.length*4; i++) {
				str += tab.charAt((binarray[i >> 2] >> ((i%4)*8+4)) & 0xF) + tab.charAt((binarray[i >> 2] >> ((i%4)
					*8)) & 0xF);
			}
			return str;
		}
